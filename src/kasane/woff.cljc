(ns kasane.woff
  "WOFF 1.0 web font decode. WOFF wraps an SFNT (TTF/OTF) with per-table zlib
   compression, so this reassembles a plain SFNT (decompressing each table via
   the existing inflate) and delegates to kasane.ttf — another 'free' format
   that reuses inflate + the TTF reader. WOFF2 (Brotli + transforms) is a
   separate larger effort. See ADR-2606272100."
  (:require [kasane.bytes :as b]
            [kasane.codec.inflate :as inflate]
            [kasane.ttf :as ttf]))
                      ...
                      (mapv (fn [i]
                              (let [o   (+ 44 (* i 20))
                                ...
                                {:tag  (b/bytes->ascii (subvec bv o (+ o 4)))
                                 :csum (be bv (+ o 16) 4)
                                 :olen olen
                                 :data (if (= clen olen) raw (inflate/inflate raw))})))
                      (sort-by :tag)
                      vec)
...
(defn parse
  "Decode WOFF1 `data` → SFNT metadata (same shape as kasane.ttf/parse)."
  [data]
  (assoc (ttf/parse (->sfnt data)) :woff true))
